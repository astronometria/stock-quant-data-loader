...
def run() -> None:
    """Apply the manual override layer additively and idempotently."""
    configure_logging()
    ...
    try:
        ...
        conn.execute("DROP TABLE IF EXISTS tmp_manual_missing_open_refs")
        conn.execute(
            """
        ...
        conn.execute(
            ...
            staged AS (
                SELECT
                    ...
                    mapped_symbol,
                    exchange_name,
                    ROW_NUMBER() OVER (ORDER BY mapped_symbol, instrument_id) AS rn
                FROM tmp_manual_missing_open_refs
            )
            SELECT
        ...
        added_symbol_count = conn.execute(
            "SELECT COUNT(*) FROM tmp_manual_missing_open_refs"
        ).fetchone()[0]
